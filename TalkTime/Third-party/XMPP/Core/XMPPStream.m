...
- (BOOL)registerWithPassword:(NSString *)password error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		XMPPLogSend(@"registerWithPassword SEND: %@", outgoingStr);
		numberOfBytesSent += [outgoingData length];
...
- (void)sendOpeningNegotiation
{
	...
	if (![self didStartNegotiation])
	{
		...
		XMPPLogSend(@"sendOpeningNegotiation1 SEND: %@", s1);
		numberOfBytesSent += [outgoingData length];
	...
	XMPPLogSend(@"sendOpeningNegotiation2 SEND: %@", s2);
	numberOfBytesSent += [outgoingData length];
...
- (void)handleStreamFeatures
{
	...
	if (f_bind)
	{
		...
		if ([requestedResource length] > 0)
		{
			...
			XMPPLogSend(@"handleStreamFeatures1 SEND: %@", outgoingStr);
			numberOfBytesSent += [outgoingData length];
		...
		else
		{
			...
			XMPPLogSend(@"handleStreamFeatures2 SEND: %@", outgoingStr);
			numberOfBytesSent += [outgoingData length];
...
- (void)handleBinding:(NSXMLElement *)response
{
	...
	if (r_jid)
	{
		...
		if (f_session)
		{
			...
			XMPPLogSend(@"handleBinding SEND: %@", outgoingStr);
			numberOfBytesSent += [outgoingData length];
...
- (void)continueHandleBinding:(NSString *)alternativeResource
{
	if ([alternativeResource length] > 0)
	{
		...
		XMPPLogSend(@"continueHandleBinding1 SEND: %@", outgoingStr);
		numberOfBytesSent += [outgoingData length];
	...
	else
	{
		...
		XMPPLogSend(@"continueHandleBinding2 SEND: %@", outgoingStr);
		numberOfBytesSent += [outgoingData length];
...
- (void)socket:(GCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag
{
	...
	XMPPLogRecvPre(@"RECV  socket : %@", [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding]);
...
- (void)xmppParser:(XMPPParser *)sender didReadRoot:(NSXMLElement *)root
{
	...
	XMPPLogTrace();
	XMPPLogRecvPost(@"RECV xmppParser didReadRoot  : %@", [root compactXMLString]);
	...
	if ([self isP2P])
	{
		...
		if ([self isP2PRecipient])
		{
			...
			XMPPLogSend(@"xmppParser isP2P SEND: %@", outgoingStr);
			numberOfBytesSent += [outgoingData length];
...
- (void)xmppParser:(XMPPParser *)sender didReadElement:(NSXMLElement *)element
{
	...
	XMPPLogTrace();
	XMPPLogRecvPost(@"RECV xmppParser didReadElement: %@", [element compactXMLString]);
